...
class Output2Excl(object):
    ...
    def save_rs(cls, sheet, table_name, column_name, sentence, et, line_no):
        sheet.write(line_no, 0, table_name)
        sheet.write(line_no, 1, column_name)
        sheet.write(line_no, 2, sentence)

        et_value = et.get_rs()
        et_values = et_value.split('@')
        for ele_index, ele in enumerate(et_values):
            sheet.write(line_no, 3 + ele_index, ele)
...
class EntityStructure(object):
    ...
    def set_value(self, et):
        ...
        self.normalized_value = et['normalized_value']
        self.father_value = et['father_value']

        original_value = self.original_value.split(']')[-1]
        if self.father_value != original_value:
            self.is_big = '否'

        self.label = et['label']
        self.sub_label = et['sub_label']
    ...
    def get_rs(self):
        ...
        tmp_rs.append(self.normalized_value)
        tmp_rs.append(self.father_value)
        tmp_rs.append(self.is_big)
        tmp_rs.append(self.label)
        tmp_rs.append(self.sub_label)
